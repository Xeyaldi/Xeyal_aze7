...
async def reload_bot(client, message):
    ...
    chat_status[chat_id] = True # Default aktiv
    await message.reply_text("🔄 **Sistem yeniləndi!**\nAdmin siyahısı və bazaya qoşulma yoxlanıldı. İndi əmrləri təkrar yoxlayın.")
...
async def chatbot_logic(client, message):
    ...
    try:
        if message.text or message.sticker or message.voice:
            ...
            cur.execute("INSERT INTO brain (msg_type, content, file_id, chat_id, user_id, first_name) VALUES (%s,%s,%s,%s,%s,%s)", (m_type, content, file_id, chat_id, message.from_user.id, message.from_user.first_name))
