...
# --- YENİ: KÖHNƏ MESAJLARI SKAN EDƏN FUNKSİYA ---
...
async def full_scan_history(client, message):
    ...
    m_status = await message.reply_text("🚀 **Tarixçə skan edilir...**\nBu əməliyyat qrupdakı bütün köhnə mesajları bir-bir oxuyur.")
    
    count = 0
    try:
        # Pyrogram-ın get_chat_history funksiyası ilə bütün keçmişi oxuyuruq
        async for msg in client.get_chat_history(chat_id):
            if msg.from_user and not msg.from_user.is_bot:
                u_id = msg.from_user.id
                # Plugins-dəki user_stats-a xal olaraq əlavə edirik
                from plugins import user_stats
                if chat_id not in user_stats: user_stats[chat_id] = {}
                user_stats[chat_id][u_id] = user_stats[chat_id].get(u_id, 0) + 1
                count += 1
                
                # Hər 1000 mesajda bir ekranda yeniləyirik
                if count % 1000 == 0:
                    await m_status.edit(f"🔍 Skan davam edir...\n✅ Analiz edilən: `{count}` mesaj")
        
        await m_status.edit(f"✅ **Skan tamamlandı!**\nCəmi `{count}` köhnə mesaj tapıldı və sıralamaya əlavə edildi.")
    except FloodWait as e:
        await asyncio.sleep(e.value) # Telegram limiti aşarsa gözləyir
...
async def start_bot():
    ...
    await app.set_bot_commands([
        ...
        BotCommand("fullscan", "Keçmiş mesajları skan et"),
