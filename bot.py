...
# --- START MESAJI (DƏQİQ FONT VƏ MƏTN) ---
...
async def sahiba_callback(client, callback_query):
    ...
    await callback_query.edit_message_caption(
        caption=(
            ...
            "İstənilən bir mesajı yazıb və ya reply edib `/yonlendir` yazın.\n\n"
            "**Nümunə:** `/yonlendir Salam Qrup Üzvləri!`"
        ),
...
async def broadcast_func(client, message):
    ...
    status_msg = await message.reply_text("📢 Mesaj yönləndirilir...")
    conn = get_db_connection(); cur = conn.cursor()
    ...
    await status_msg.edit(f"✅ Yönləndirmə tamamlandı: {success} qrupa göndərildi.")
...
async def chatbot_logic(client, message):
    ...
    if bot_me.first_name.lower() in message.text.lower():
        await message.reply_text(random.choice(CB_SOZLER))

# --- DİGƏR ---
@app.on_message(filters.command("help"))
async def help_cmd(client, message):
    await message.reply_text("🎮 **OYUNLAR:** /basket, /futbol, /dart, /slot, /dice\n📢 **TAĞ:** /tag, /utag, /flagtag, /tektag\n🛑 **DAYANDIR:** /tagstop\n🆔 **ID:** /id")
