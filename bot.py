...
# --- YENİ SIRALAMA PLUGİNİ (ƏLAVƏ OLUNDU) ---
@app.on_message(filters.command("topsiralama") & filters.group)
async def top_thirteen_plugin(client, message):
    try:
        conn = get_db_connection(); cur = conn.cursor()
        cur.execute("SELECT user_id, msg_count FROM user_stats WHERE chat_id = %s ORDER BY msg_count DESC LIMIT 13", (message.chat.id,))
        rows = cur.fetchall()
        if not rows:
            return await message.reply_text("🪖 Siyahı hələ boşdur.")
        text = "🏆 **QRUPUN AKTİV 13 ÜZVÜ**\n⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n\n"
        medals = {1: "🥇", 2: "🥈", 3: "🥉"}
        for i, (u_id, count) in enumerate(rows, 1):
            medal = medals.get(i, f" {i}. ")
            try:
                user = await client.get_users(u_id)
                mention = f"[{user.first_name}](tg://user?id={u_id})"
            except: mention = f"İstifadəçi"
            if count > 10000: rank = "💎 Marşal"
            elif count > 5000: rank = "🌟 General"
            elif count > 2000: rank = "🎖️ Polkovnik"
            elif count > 1000: rank = "🏅 Mayor"
            elif count > 50: rank = "🔫 Çavuş"
            else: rank = "🛡️ Sıravi"
            text += f"{medal} **{mention}**\n╰─ {rank} • `{count}` mesaj\n\n"
        text += "⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n📊 *Statistika avtomatik yenilənir.*"
        await message.reply_text(text)
        cur.close(); conn.close()
    except Exception as e: print(f"Top xətası: {e}")
...
async def start_bot():
    ...
    await app.set_bot_commands([
        ...
        BotCommand("topsiralama", "Aktiv 13 nəfər"),
